...
def update_db(fasta_file):
    ...
    if fsuffix == '.fsa':
        if fname not in os.listdir(database_path):
            ...
            seq_type = cfunc.check_sequence_type(dest_file)
            if seq_type == 'DNA':
                Blaster.makeblastdb(dest_file, blastdb_out, db_type='nucl')
            elif seq_type == 'Amino Acid':
                Blaster.makeblastdb(dest_file, blastdb_out, db_type='prot')
            else:
                print('Unknown sequence type, exit ...')
                sys.exit(1)
        else:
...
def main():
    ...
    if args.subcommand is None:
        ...
        if database in exist_database:
            ...
            print(
                f'Could not found {database} in {exist_database}, Please check your input or view database list using "ResBlaster show_db"')
            sys.exit(1)
